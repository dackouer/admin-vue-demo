<script setup lang="ts">
    ...
    const form = reactive({id:0,title:'',pic:''})
    ...
    const getTableData = async () => {
        ActivityCate.show(search).then((res)=>{
            if(res.data.code === 0){
    ...
    const batchDel = (table:string) => {
        ...
        ActivityCate.del(selectID).then((res)=>{
            if(res.data.code === 0){
                ElMessage('删除成功')
                setTimeout(()=>{
                    window.location.reload()
    ...
    // 提交表单
    const submit = async () => {
        ActivityCate.save(form).then((res)=>{
            if(res.data.code === 0){
    ...
    <el-dialog v-model="dialogFormVisible" :title="(form?.id ? '修改' : '新增') + tableData?.title.replace('管理','')" width="700px">
        <el-form ref="ruleFormRef" :model="form" size="large" :label-width="formLabelWidth">
            ...
            <el-form-item label="图片：">
                <el-col :span="14">
                    <el-upload v-model:file-list="fileList" class="upload-demo" :action="uploadAction+'?path=Activity/activity'" :limit="1" :on-exceed="handleExceed" :on-success="uploadSuccessPic" :on-preview="handlePreviewPic" :on-remove="handleRemovePic" list-type="picture">
