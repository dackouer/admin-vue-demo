<script setup lang="ts">
    ...
    const form = reactive({id:0,title:'',logo:''})
    ...
    watch(keyid,(val,value)=>{
        if(val){
            Brand.get(val).then((res)=>{
                if(res.data.code === 0){
                    const data = JSON.parse(JSON.stringify(res.data.data))
                    form.id = data.id
                    form.title = data.title
                    form.logo = data.logo
                    fileList.value.push({name:'',url:form.logo})
                }else{
    ...
    <el-dialog v-model="dialogFormVisible" :title="(form?.id ? '修改' : '新增') + tableData?.title.replace('管理','')" width="700px">
        <el-form ref="ruleFormRef" :model="form" size="large" :label-width="formLabelWidth">
            ...
            <el-form-item label="Logo">
                <el-col :span="12">
                    <el-upload v-model:file-list="fileList" class="upload-demo" :action="uploadAction+'?path=brand'" :limit="1" :on-exceed="handleExceed" :on-success="uploadSuccessPic" :on-preview="handlePreviewPic" :on-remove="handleRemovePic" list-type="picture">
                        <el-button type="primary" class="bg-blue-400">上传Logo</el-button>
                    </el-upload>
                </el-col>
            </el-form-item>
        </el-form>
