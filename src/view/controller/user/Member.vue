<script setup lang="ts">
    ...
    const roleData = ref()
    const search = reactive({keyword:'',page:1,pagesize:10,pid:5})
    const selectID = ref([])
...
<template>
    <el-container class="bg-white">
        <el-header class="flex justify-between bg-white border-b py-4">
            <div class="flex justify-center items-center">
                ...
                <el-link>会员管理</el-link>
            </div>
            <div>
                <el-button type="primary" plain @click="add(tableData?.table)">注册新会员</el-button>
            </div>
        </el-header>
    ...
    <!-- action form -->
    <el-dialog v-model="dialogFormVisible" :title="(form?.id ? '修改' : '注册新') + '会员'" width="700px">
