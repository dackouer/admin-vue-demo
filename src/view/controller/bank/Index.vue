<script setup lang="ts">
    ...
    const form = reactive({id:0,title:'',full_name:'',bank_en_name:'',bank_code:'',website:'',hotline:'',is_popular:0})
    ...
    watch(keyid,(val,value)=>{
        if(val){
            Bank.get(val).then((res)=>{
                if(res.data.code === 0){
                    const data = JSON.parse(JSON.stringify(res.data.data))
                    form.id = data.id
                    form.title = data.title
                    form.full_name = data.full_name
                    form.bank_en_name = data.bank_en_name
                    form.bank_code = data.bank_code
                    form.website = data.website
                    form.hotline = data.hotline
                    form.is_popular = data.is_popular
                }else{
    ...
    <el-dialog v-model="dialogFormVisible" :title="(form?.id ? '修改' : '新增') + tableData?.title.replace('管理','')" width="700px">
        <el-form ref="ruleFormRef" :model="form" size="large" :label-width="formLabelWidth">
            ...
            <el-form-item label="全称：">
                <el-col :span="14">
                    <el-input v-model="form.full_name" autocomplete="off" />
                </el-col>
            </el-form-item>
            <el-form-item label="英文名称：">
                <el-col :span="14">
                    <el-input v-model="form.bank_en_name" autocomplete="off" />
                </el-col>
            </el-form-item>
            <el-form-item label="银行编码：">
                <el-col :span="14">
                    <el-input v-model="form.bank_code" autocomplete="off" />
                </el-col>
            </el-form-item>
            ...
            <el-form-item label="客服热线：">
                <el-col :span="14">
                    <el-input v-model="form.hotline" autocomplete="off" />
                </el-col>
            </el-form-item>
            <el-form-item label="设为常用：">
                <el-switch v-model="form.is_popular" :active-value="1" :inactive-value="0" />
            </el-form-item>
        </el-form>
        <template #footer>
