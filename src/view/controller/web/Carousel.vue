<script setup lang="ts">
    ...
    const form = reactive({id:0,title:'',img:'',url:'https://www.yijiantea.cn',opentype:'navigate',sort:1})
        ...
        if(val){
            Carousel.get(val).then((res)=>{
                if(res.data.code === 0){
                    ...
                    form.sort = data.sort
                    fileList.value.push({name:'',url:form.img})
                }else{
            ...
            form.title = ''
            form.img = ''
            form.url = 'https://www.yijiantea.cn'
            form.opentype = 'navigate'
            form.sort = 1
        }
    })
    ...
    const batchDel = (table:string) => {
        ...
        Carousel.del(selectID).then((res)=>{
            if(res.data.code === 0){
                ElMessage('删除成功')
                setTimeout(()=>{
                    window.location.reload()
    ...
    <el-dialog v-model="dialogFormVisible" :title="(form?.id ? '修改' : '新增') + tableData?.title.replace('管理','')" width="700px">
        <el-form ref="ruleFormRef" :model="form" size="large" :label-width="formLabelWidth">
            ...
            <el-form-item label="图片">
                <el-col :span="16">
                    <el-upload v-model:file-list="fileList" class="upload-demo" :action="uploadAction+'?path=creator'" :limit="1" :on-exceed="handleExceed" :on-success="uploadSuccessPic" :on-preview="handlePreviewPic" :on-remove="handleRemovePic" list-type="picture">
            ...
            <el-form-item label="链接方式">
                <el-col :span="16">
                    <el-radio-group v-model="form.opentype">
                        <el-radio label="navigate">内链</el-radio>
                        <el-radio label="click">外链</el-radio>
                    </el-radio-group>
                </el-col>
            </el-form-item>
