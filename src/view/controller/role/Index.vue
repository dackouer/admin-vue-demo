<script setup lang="ts">
    ...
    const form = reactive({id:0,title:'',pic:'',pid:0,level:1,is_admin:0,is_allow_login:0,is_default:0,sort:1})
        ...
        if(val){
            Role.get(val).then((res)=>{
                if(res.data.code === 0){
                    ...
                    form.is_allow_login = data.is_allow_login
    ...
    <el-dialog v-model="dialogFormVisible" :title="(form?.id ? '修改' : '新增') + tableData?.title.replace('管理','')" width="700px">
        <el-form ref="ruleFormRef" :model="form" size="large" :label-width="formLabelWidth">
            ...
            <el-form-item label="图片：">
                <el-col :span="14">
                    <el-upload v-model:file-list="fileList" class="upload-demo" :action="uploadAction+'?path=role'" :limit="1" :on-exceed="handleExceed" :on-success="uploadSuccessPic" :on-preview="handlePreviewPic" :on-remove="handleRemovePic" list-type="picture">
                        <el-button type="primary" class="bg-blue-400">上传图片</el-button>
                    </el-upload>
                </el-col>
            </el-form-item>
            <el-form-item label="是否管理员：" v-if="form.pid">
                <el-col :span="14">
                    <el-switch v-model="form.is_admin" :active-value="1" :inactive-value="0" />
                </el-col>
            </el-form-item>
            <el-form-item label="允许登录后台：" v-if="form.pid">
                <el-col :span="14">
                    <el-switch v-model="form.is_allow_login" :active-value="1" :inactive-value="0" />
                </el-col>
            </el-form-item>
            <el-form-item label="默认注册：" v-if="form.pid">
                <el-col :span="14">
