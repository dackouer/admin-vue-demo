<script setup lang="ts">
    ...
    const form = reactive({id:0,title:'',handle_name:''})
    ...
    watch(keyid,(val,value)=>{
        if(val){
            Handle.get(val).then((res)=>{
                if(res.data.code === 0){
                    const data = JSON.parse(JSON.stringify(res.data.data))
                    form.id = data.id
                    form.title = data.title
                    form.handle_name = data.handle_name
                }else{
    ...
    <el-dialog v-model="dialogFormVisible" :title="(form?.id ? '修改' : '新增') + tableData?.title.replace('管理','')" width="700px">
        <el-form ref="ruleFormRef" :model="form" size="large" :label-width="formLabelWidth">
            ...
            <el-form-item label="标识符：">
                <el-col :span="14">
                    <el-input v-model="form.handle_name" autocomplete="off" />
                </el-col>
            </el-form-item>
        </el-form>
        <template #footer>
